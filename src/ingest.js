// src/ingest.js
// CLI entry point: reads a CSV or Excel file from disk into the database.
//
// Usage:
//   npm run ingest                      -> reads ./atlas_inventory.csv
//   npm run ingest ./path/to/file.csv   -> reads a specific file
//   npm run ingest ./path/to/file.xlsx  -> Excel works too
//
...
async function main() {
  let result;
  try {
    result = await ingestFile(fs.readFileSync(csvPath));
  } catch (e) {
    db.close();
  ...
  console.log("\n===== Ingestion Summary =====");
  console.log(`Format       : ${result.format === "excel" ? "Excel" : "CSV"}`);
